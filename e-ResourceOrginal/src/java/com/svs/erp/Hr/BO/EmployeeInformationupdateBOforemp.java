...
import com.svs.erp.Hr.DAO.EmployeeInformationupdateDAOemp;
import com.svs.util.ConvertStackTracetoString;

public class EmployeeInformationupdateBOforemp extends HttpServlet
{
int empno;
		String empnoo;

		Collection employee_coll=new ArrayList();
		EmployeeInformationupdateDAOemp empupdate=new EmployeeInformationupdateDAOemp();
		Connection con=null;
		ConnectionUtils connectionUtils=new ConnectionUtils();
		final static Logger logger = Logger.getLogger(EmployeeInformationupdateBOforemp.class.getName());
...
employee_coll.add(req.getParameter("empno"));//a[33]
	...
	try
	{
		empnoo=empupdate.insert(employee_coll);
		 ...
		 session.setAttribute("empno1" , "Dear "+name+", Your Details Updated Successfully...");
		 ////////System.out.println("Empno in session"+empnoo);
		employee_coll.clear();
		res.sendRedirect("employeedetailsupdateform");
	}
	catch (Exception e)
	{
