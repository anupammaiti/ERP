...
import java.sql.*;
public class Loadtomysqlinoutold extends HttpServlet
{
	private static Logger logger =Logger.getLogger(Loadtomysqlinoutold.class);
...

	{
		...
		try{
			...
			//////System.out.println("Loadtomysqlinoutold::ConnectionPool----->>>>Before Open --->>>>Test");
			conn=connectionUtils.getDBConnection();
			//////System.out.println("Loadtomysqlinoutold::ConnectionPool----->>>>After Open --->>>>Test");
		...
		//////System.out.println("totalhrs"+totalhrs);
			...
			rs4 = st4.executeQuery("select empid,empname,department,date,AVG(TIMEDIFF(last,first)) from empinout");
			String empid4=null;
			...
			while(rs4.next())
			{
				...
				String s5="insert into empinout1(empid,empname,department,designation,RM,HOD,date,first,last,tothrs)select e.empid,e.empname,e.department,e1.designation,e1.reportingto,e1.reportingto,e.date,e.first,e.last,e.totalhrs=(select TIMEDIFF(last,first) from empinout a where a.empid="+empid4+") from empinout e,employee e1";
				pr5=conn.prepareStatement(s5);
				//pr5.setString(1,empid4);
				//pr5.setString(2,empname4);
				//pr5.setString(3,department4);
				////////System.out.println("empinout1"+empid4+"**"+totalhrs4);
				// pr5.setString(4,designation4);
				//pr5.setString(5,rm4);
				//pr5.setString(6,rm4);
				// pr5.setString(7,date4);
				// pr5.setLong(8,first);
				// pr5.setLong(9,last);
			...
			try
			{
				//////System.out.println("Loadtomysqlinoutold::ConnectionPool--Before Close --->>>>Test");
				if (conn != null) connectionUtils.setDBClose();
				//////System.out.println("Loadtomysqlinoutold::ConnectionPool--After Close--->>>>Test");
