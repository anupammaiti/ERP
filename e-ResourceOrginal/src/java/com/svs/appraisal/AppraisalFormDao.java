...
public class AppraisalFormDao {
	...
	int i;
	final static Logger logger = Logger.getLogger(AppraisalFormDao.class);
	ConvertStackTracetoString util_stacktrace=new ConvertStackTracetoString();
	//DataBaseConnection connection=new DataBaseConnection();
	ConnectionUtils connectionUtils=new ConnectionUtils();
	
	public AppraisalFormDao(){
		try{
	...
	public String insertappraisalform(Collection appraisal_col) throws Exception{
		
		try
		{
			for(iterator=appraisal_col.iterator();iterator.hasNext();)
			{
				for( i=0;i<appraisal_col.size();i++)
				{
			...
			pstmt=con.prepareStatement("insert into appraisee values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	try{
		...
		pstmt_Checking=con.prepareStatement("select aPPraiseeId from appraisee where aPPraiseeId=?");
		pstmt_Checking.setString(1,aPPraiseeId);
		rs_Checking=pstmt_Checking.executeQuery();
		while(rs_Checking.next()){
		counter=true;
		}
		//////System.out.println("AppraiseeForm is Duplicate"+counter);
	}
	catch(Exception e){
		logger.error(util_stacktrace.sendingErrorAsString(e));
	}
