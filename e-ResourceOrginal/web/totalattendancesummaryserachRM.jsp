...
String hod=request.getParameter("hod");
  ...
  String query;
   ...
   //conn=cc.connection1();
	...
	//////System.out.println("totalattendancesummaryserachRM.jsp::ConnectionPool----->>>>Before Open --->>>>Test");
	conn=connectionUtils.getDBConnection();
	//////System.out.println("totalattendancesummaryserachRM.jsp::ConnectionPool----->>>>After Open --->>>>Test");
	   
  if((rm!=null) && (rm!="") && (hod.equals("")) )
	{
	////////System.out.println("company"+companyname);
	//////System.out.println("8");
	   query="select distinct empid,empname,department,designation,rm,hod from  empinout1 where (rm='"+rm+"') and date between ? and ?";
	  // //////System.out.println("company"+companyname);
	...
	////////System.out.println("8");
	   query="select distinct empid,empname,department,designation,rm,hod from  empinout1 where (hod='"+hod+"') and date between ? and ?";
	  // //////System.out.println("company"+companyname);
            pstmt=conn.prepareStatement(query);
			pstmt.setString(1,fromdate);
			pstmt.setString(2,todate);
		...
		{
		query="select distinct empid,empname,department,designation,rm,hod from empinout1 where date between ? and ? order by empid";
   ...
   while(rs.next())
	 ...
	 c++;
	   ...
	   <tr   onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'" class="tr1">
		...
		<td>
      <a href="TotalattandancesummaryRM?empno=<%=rs.getString(1)%>" target="_blank"  id="anchorcolor" title="Click Here To View Details"><img src="images/report1.png" align="absmiddle">&nbsp;<%=rs.getString(1)%></a>
</td>
...
try
			{
				//////System.out.println("totalattendancesummaryserachRM.jsp::ConnectionPool--Before Close --->>>>Test");
				if (conn != null) connectionUtils.setDBClose();
				//////System.out.println("totalattendancesummaryserachRM.jsp::ConnectionPool--After Close--->>>>Test");
